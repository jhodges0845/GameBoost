...
namespace GameBoost.Rendering.Tests
{
    ...
    internal class Camera2DTests
    {
        ...
        {
            var camera = new Camera2D(Vector2D.Zero, 1f, 800f, 600f);
            var moved = camera.Move(new Vector2D(10f, 20f));
        ...
        {
            var camera = new Camera2D(Vector2D.Zero, 1f, 800f, 600f);
            var sprite = new Sprite(new Vector2D(0f, 0f), 10f, 10f);
            var farSprite = new Sprite(new Vector2D(500f, 500f), 10f, 10f);
